...
class RdfBuilder extends Readable {
  ...
  import (cube) {
    ...
    const shapeTerm = rdf.namedNode(url.resolve(this.baseIRI, 'shape'))
    const observationSetTerm = rdf.namedNode(url.resolve(this.baseIRI, 'observation/'))
    ...
    for (const observation of cube.observations()) {
      const observationTerm = rdf.namedNode(url.resolve(this.baseIRI, `observation/${observation.row}`))
