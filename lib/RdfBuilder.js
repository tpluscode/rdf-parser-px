...
class RdfBuilder extends Readable {
  ...
  import (cube) {
    ...
    if (creationDate) {
      this.push(rdf.quad(datasetTerm, ns.schema.dateCreated, rdf.literal(DateTime.fromFormat(creationDate.values[0], 'yyyyMMdd mm:ss').toISO(), ns.xsd.dateTime)))
    }

    const lastUpdated = cube.value('LAST-UPDATED')

    if (lastUpdated) {
      this.push(rdf.quad(datasetTerm, ns.schema.dateModified, rdf.literal(DateTime.fromFormat(lastUpdated.values[0], 'yyyyMMdd mm:ss').toISO(), ns.xsd.dateTime)))
    }
