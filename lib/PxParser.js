...
PxParser.token = {
  KEY: new RegExp('^([^=]*)='),
  KEY_PARTS: new RegExp('([^\\[;^\\("]*)(\\[([^\\]]*)\\])?(\\("([^"]*)"\\))?'),
  LIST: new RegExp('^[\\s]*(,)[\\s]*'),
  QUOTED_VALUE_START: new RegExp('^[\\s]*(")'),
  QUOTED_VALUE: new RegExp('^([^"]*)"'),
  UNQUOTED_VALUE: new RegExp('^[\\s]*([^;]*)'),
  QUOTED_DATA_VALUE: new RegExp('^"([^\\s^"^;]*)"[\\s]*'),
  UNQUOTED_DATA_VALUE: new RegExp('^([^\\s^;]*)[\\s]*'),
  END: new RegExp('^(;)[\\s]*')
}
